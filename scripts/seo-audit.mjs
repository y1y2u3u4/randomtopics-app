...
const checks = [
  ...
  { path: "/question-of-the-day", index: true, canonical: "/question-of-the-day", titleMax: 70, titleHas: "Question of the Day", bodyHas: ["Today’s question for", "One question per local day", "Funny Question of the Day", "100 audience-safe funny prompts", "Copy for group chat", "Plan 5 classroom questions", "Plan 5 team questions", 'id="question-ideas"', 'href="#question-ideas"', ...["classroom", "work", "kids", "deep", "funny"].flatMap((id) => [`id="ideas-${id}"`, `href="#ideas-${id}"`])], bodyOccurrences: [{ needle: 'data-qotd-question="true"', exact: 120 }] },
  { path: "/question-of-the-day-for-students", index: true, canonical: "/question-of-the-day-for-students", titleMax: 60, titleHas: "Question of the Day for Students", enOnlyHreflang: true, bodyHas: ["180", "Today's Classroom Question", "Build a five-day plan", "Build weekly plan", "Start with current question", "Save ☆", '"@type":"WebApplication"'], bodyNotHas: ['"@type":"FAQPage"'], bodyOccurrences: [{ needle: 'class="rounded-xl border border-white/10 p-4 sm:p-5"', exact: 180 }] },
  { path: "/question-of-the-day-for-work", index: true, canonical: "/question-of-the-day-for-work", titleMax: 60, titleHas: "Question of the Day for Work", enOnlyHreflang: true, bodyHas: ["120", "Today's Team Question", "Copy for Slack / Teams", "Build a five-day plan", "Start with current question", "Save ☆", '"@type":"WebApplication"'], bodyNotHas: ['"@type":"FAQPage"'], bodyOccurrences: [{ needle: 'class="rounded-xl border border-white/10 p-4 sm:p-5"', exact: 120 }] },
  ...
  { path: "/conversation", index: true, canonical: "/conversation", titleMax: 65, titleHas: "Conversation Topic Generator", hreflang: true, bodyHas: ["Popular Conversation Starter Collections", "Deep Conversation Question Generator", 'href="/hot-seat-questions"', 'href="/question-generator"', 'href="/topics/two-truths-and-a-lie-ideas"'], bodyOccurrences: [{ needle: 'href="/topics/', min: 6 }] },
  ...
  { path: "/es/topics/controversial-topics-to-discuss", index: true, canonical: "/es/topics/controversial-topics-to-discuss", titleMax: 60, titleHas: "Temas Controversiales para Debatir", es: true, hreflang: true, webApplicationCanonical: true, bodyHas: ["temas controversiales", "Abrir el Generador de Debate", "Prepara un debate con dos perspectivas", "Elegir un tema de debate", "Perspectiva A:", "Perspectiva B:", "Para profundizar:"], bodyOccurrences: [{ needle: 'data-discussion-card="true"', exact: 55 }] },
  ...
  { path: "/topics/toastmasters-table-topics", index: true, canonical: "/topics/toastmasters-table-topics", titleMax: 60, titleHas: "Toastmasters Table Topics", bodyHas: ["Updated:", "September 4, 2026", "Practice a Random Table Topic", "PREP response structure", "Speech Timer", "Draw a Table Topic", 'id="generator-toastmasters_article"', 'href="#generator-toastmasters_article"', "Run a Complete Table Topics Drill", "Open the Table Topics Generator + Timer", "Run live rounds with the focused generator", "Green 1:00", "Yellow 1:30", "Red 2:00"] },
  { path: "/es/most-likely-to", index: true, canonical: "/es/most-likely-to", titleMax: 76, titleHas: "Quién Es Más Probable", es: true, hreflang: true, bodyHas: ["Elegir categoría y preparar una ronda", "Son dos mazos distintos"] },
  { path: "/es/topics/most-likely-to-questions", index: true, canonical: "/es/topics/most-likely-to-questions", titleMax: 65, titleHas: "100 Preguntas de Quién Es Más Probable", es: true, hreflang: true, bodyHas: ["Preparar una ronda de esta categoría", "Preguntas fuertes de Quién es más probable para amigos", "Colecciones de Quién es más probable", "Juega a Quién Es Más Probable", "Preparar ronda", 'id="generator-es_most_likely_article"', 'href="#generator-es_most_likely_article"', "Preguntas por ronda", "Elige una categoría"], bodyOccurrences: [{ needle: 'class="flex items-start gap-3"', exact: 100 }] },
...
async function checkSitemap() {
  ...
  for (const path of [
    ...
    const marker = `<loc>${canonicalOrigin}${path}</loc>`;
    const entryTail = xml.split(marker)[1]?.split("</url>")[0] || "";
    if (!entryTail.includes("<lastmod>2026-09-01")) fail(`sitemap: stale lastmod for ${path}`);
  }
  const updatedParty = xml.split(`<loc>${canonicalOrigin}/es/topics/most-likely-to-questions</loc>`)[1]?.split("</url>")[0] || "";
  if (!updatedParty.includes("<lastmod>2026-09-15")) fail("sitemap: stale Spanish party lastmod");
  const updatedDiscussion = xml.split(`<loc>${canonicalOrigin}/es/topics/controversial-topics-to-discuss</loc>`)[1]?.split("</url>")[0] || "";
  if (!updatedDiscussion.includes("<lastmod>2026-09-12")) fail("sitemap: stale Spanish discussion lastmod");
  return urls.length;
}
