...
export default function ArticleGeneratorEntry({ source, locale = "en", surface }: {
  ...
  return (
    <div className="glass-card my-6 p-5 text-center border-[var(--neon-cyan)]/20">
      <p className="mb-3 text-sm text-[var(--text-secondary)]">
        {isSpanish ? "¿Listos para jugar? Saca una pregunta de la categoría que elegiste." : "Ready to try one? Draw a prompt from your selected category."}
      </p>
      <a href={`#generator-${source}`} className="btn-generate inline-flex min-h-11 text-sm" onClick={(event) => {
        ...
        <span aria-hidden="true">🎲</span> {isSpanish ? "Sacar la siguiente pregunta" : "Draw the next prompt"}
      </a>
    </div>
  );
}
