...
export default function EthicsCardActions({ card }: { card: EthicsDiscussionCard }) {
  ...
  return <div className="mt-5 space-y-3">
    ...
    <PrintButton heading={card.title} items={[card.scenario, ...card.options.map((option, i) => `Option ${i === 0 ? "A" : "B"}: ${option.choice}. Reason: ${option.reason} Trade-off: ${option.cost}`), ...card.followUps]}
      intro="Discuss both positions, consider another option, and let anyone pass. Fictional discussion exercise, not professional advice." footerNote="One discussion card from randomtopics.app — scenario, two positions and two follow-ups." label="Print this discussion card" />
  </div>;
}
