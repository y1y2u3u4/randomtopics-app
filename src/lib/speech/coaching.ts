...
Quote exact short contiguous substrings (prefer 5–20 words), never paraphrases or joined passages. Preserve the transcript's punctuation. Every met criterion must have a non-empty supporting quote from the current transcript. Empty quote is appropriate for an absent element. Keep each explanation to one sentence, at most 30 words. Address the speaker as “you”. Name what the text does in plain English; do not write generic praise or a new coaching task. Mark unclear rather than inventing evidence. Return plain text inside JSON fields.`;
