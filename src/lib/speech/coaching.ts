...
Quote exact short contiguous substrings (prefer 5–15 words), never paraphrases or joined passages. Preserve the transcript's punctuation. Every met criterion must have a non-empty supporting quote from the current transcript. Empty quote is appropriate for an absent element. Each quote has a hard limit of 240 characters including spaces and punctuation: aim below 180 characters and select only the relevant excerpt, not an entire paragraph. Each criterion explanation has a hard limit of 300 characters: aim below 200 characters, in one complete sentence of at most 25 words. Address the speaker as “you”. Name what the text does in plain English; do not write generic praise or a new coaching task. Mark unclear rather than inventing evidence. Return plain text inside JSON fields.`;

export function coachingInstruction(previous: SpeechFeedback | undefined, focused: boolean) {
  const comparison = `Compare ONLY the previous practice goal, using exact evidence from both answers. Do not move the goalposts or require a new unrelated skill. improved means that goal is now better supported; similar, mixed and insufficient_evidence are honest alternatives. Do not guarantee improvement. An empty beforeQuote is allowed only if the saved assessment says the target was missing; explain the absence. An afterQuote is required unless evidence is insufficient. Both beforeQuote and afterQuote must stay within 240 characters each; aim below 180. The comparison explanation must stay within 400 characters; aim below 250. All character limits include spaces and punctuation.`;
