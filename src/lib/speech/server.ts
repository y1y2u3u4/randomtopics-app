          ...
          messages: [
            { role: "system", content: instruction + (attempt === 2
              ? name === "speech_transcript"
                ...
                ? "\nThe previous response failed evidence validation. Generate a fresh complete JSON object matching the supplied schema. Follow the supplied quote representation: select evidence IDs when the schema provides them; otherwise copy exact, contiguous substrings. Never paraphrase, alter punctuation or combine passages. Current evidence must come from the current transcript, and beforeQuote only from the previous transcript. Every met criterion needs non-empty supporting evidence. A comparison requires a non-empty afterQuote unless its outcome is insufficient_evidence. Only beforeQuote may be empty when the saved target was missing. If the comparison lacks evidence, use insufficient_evidence and explain the limitation instead of inventing a quote."
